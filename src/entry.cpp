...
int main(int argc, char** argv)
{
  ...
  int ret = OMS_OK;
  if (init_prometheus(Config::instance().prometheus_port.val()) != OMS_OK) {
    OMS_ERROR("!!!LogProxy Exit, failed to initialize prometheus Exposer");
    ::exit(-1);
  }
  if (conf.binlog_mode.val()) {
    ret = oceanbase::binlog::BinlogManager::run_foreground();
  } else {

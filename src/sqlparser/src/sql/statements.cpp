...
// DeleteStatement
DeleteStatement::DeleteStatement() : SQLStatement(COM_DELETE), schema(nullptr), tableName(nullptr), expr(nullptr)
{}
...
DropStatement::DropStatement(DropType type)
    : SQLStatement(COM_DROP), type(type), schema(nullptr), name(nullptr), indexName(nullptr)
{}
...
AlterStatement::AlterStatement(char* name, AlterAction* action)
    : SQLStatement(COM_ALTER), schema(nullptr), ifTableExists(false), name(name), action(action)
{}
...
TransactionStatement::TransactionStatement(TransactionCommand command) : SQLStatement(COM_TRANSACTION), command(command)
{}
...
// ExecuteStatement
ExecuteStatement::ExecuteStatement() : SQLStatement(COM_EXECUTE), name(nullptr), parameters(nullptr)
{}
...
ExportStatement::ExportStatement(ImportType type)
    : SQLStatement(COM_EXPORT), type(type), filePath(nullptr), schema(nullptr), tableName(nullptr), select(nullptr)
{}
...
ImportStatement::ImportStatement(ImportType type)
    : SQLStatement(COM_IMPORT), type(type), filePath(nullptr), schema(nullptr), tableName(nullptr), whereClause(nullptr)
{}
...
// UpdateStatement
UpdateStatement::UpdateStatement() : SQLStatement(COM_UPDATE), table(nullptr), updates(nullptr), where(nullptr)
{}

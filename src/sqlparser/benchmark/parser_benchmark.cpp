...
#include "sqlparser/src/SQLParser.h"
#include "sqlparser/src/parser/bison_parser.h"
#include "sqlparser/src/parser/flex_lexer.h"
...
BENCHMARK(BM_CharacterCount)->RangeMultiplier(1 << 2)->Ranges({{1 << 5, 1 << 15}, {5, 5}});
...
static void BM_ConditionalTokens(benchmark::State& st)
{
  ...
  if (targetSize >= query.size()) {
    ...
    fprintf(
        stderr, "Too many tokens. Query too long for benchmark char limit (%lu > %lu).\n", query.size(), targetSize);
    return;
  }
...
BENCHMARK(BM_ConditionalTokens)->RangeMultiplier(1 << 2)->Ranges({{1 << 14, 1 << 14}, {1 << 2, 1 << 11}});

...
TEST(SelectExprTest)
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT a, MAX(b), CUSTOM(c, F(un)) FROM students;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL("SELECT city, COUNT(name), COUNT(DISTINCT grade) FROM students GROUP BY city;",
      kStmtSelect,
      SelectStatement,
      result,
      stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT grade, city FROM students ORDER BY grade, city DESC;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT grade, city FROM students WHERE grade BETWEEN 1 and c;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT MAX(CASE WHEN a = 'foo' THEN x ELSE 0 END) FROM test;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT CASE WHEN x = 1 THEN 1 WHEN 1.25 < x THEN 2 END FROM test;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL("SELECT CASE x WHEN 1 THEN 0 WHEN 2 THEN 3 WHEN 8 THEN 7 ELSE 4 END FROM test;",
      kStmtSelect,
      SelectStatement,
      result,
      stmt);
...
TEST(SelectJoin)
{
  TEST_PARSE_SINGLE_SQL("SELECT City.name, Product.category, SUM(price) FROM fact\
...
{
  SelectStatement* stmt;
  TEST_PARSE_SQL_QUERY("SELECT * FROM t WHERE a = 10 FOR UPDATE;"
...
{
  SelectStatement* stmt;
  TEST_PARSE_SQL_QUERY("SELECT * FROM t WHERE a = 10 FOR UPDATE NOWAIT;"
...
{
  SelectStatement* stmt;
  TEST_PARSE_SQL_QUERY("SELECT * FROM t WHERE a = 10 FOR UPDATE OF t;"

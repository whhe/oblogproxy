...
{
  SQLParserResult result;
  SQLParser::parse("CREATE TABLE dummy_table ("
                   "  c_bigint BIGINT, "
                   "  c_boolean BOOLEAN, "
                   "  c_char CHAR(42), "
                   "  c_date DATE, "
...
{
  TEST_PARSE_SINGLE_SQL("INSERT INTO students VALUES ('Max Mustermann', 12345, 'Musterhausen', 2.0)",
      kStmtInsert,
      InsertStatement,
      result,
      stmt);
...
TEST(ImportStatementTest)
{
  TEST_PARSE_SINGLE_SQL(
      "IMPORT FROM TBL FILE 'students_file' INTO students;", kStmtImport, ImportStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL("COPY students FROM 'students_file' WITH FORMAT BINARY;",
      kStmtImport,
      ImportStatement,
      import_result,
      import_stmt);
  ...
  TEST_PARSE_SINGLE_SQL("COPY students FROM 'students_file' WHERE lastname = 'Potter';",
      kStmtImport,
      ImportStatement,
      import_filter_result,
      import_filter_stmt);
  ...
  TEST_PARSE_SINGLE_SQL("COPY students TO 'students_file' WITH FORMAT CSV;",
      kStmtExport,
      ExportStatement,
      export_table_result,
      export_table_stmt);
  ...
  TEST_PARSE_SINGLE_SQL("COPY (SELECT firstname, lastname FROM students) TO 'students_file';",
      kStmtExport,
      ExportStatement,
      export_select_result,
      export_select_stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students WITH HINT(NO_CACHE, SAMPLE_RATE(10));", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students EXCEPT SELECT * FROM students_2;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students INTERSECT SELECT * FROM students_2;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students UNION SELECT * FROM students_2;", kStmtSelect, SelectStatement, result, stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students UNION ALL SELECT * FROM students_2;", kStmtSelect, SelectStatement, result, stmt);
...
TEST(NestedDifferentSetOperationsWithWithClause)
{
  ...
  ASSERT_STREQ(
      stmt->withDescriptions->back()->select->setOperations->back()->nestedSelectStatement->fromTable->name, "B");
...
TEST(NestedSetOperationsWithMultipleWithClauses)
{
  ...
  ASSERT_STREQ(
      stmt->withDescriptions->at(0)->select->setOperations->back()->nestedSelectStatement->fromTable->name, "B");
  ...
  ASSERT_STREQ(
      stmt->withDescriptions->back()->select->setOperations->back()->nestedSelectStatement->fromTable->name, "C");
...
TEST(BeginTransactionTest)
{
  {
    TEST_PARSE_SINGLE_SQL(
        "BEGIN TRANSACTION;", kStmtTransaction, TransactionStatement, transaction_result, transaction_stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "ROLLBACK TRANSACTION;", kStmtTransaction, TransactionStatement, transaction_result, transaction_stmt);
...
{
  TEST_PARSE_SINGLE_SQL(
      "COMMIT TRANSACTION;", kStmtTransaction, TransactionStatement, transaction_result, transaction_stmt);

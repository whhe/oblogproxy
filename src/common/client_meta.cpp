...
void ClientMeta::to_json(rapidjson::PrettyWriter<rapidjson::StringBuffer>& writer) const
{
  ...
  writer.String(configuration.c_str());
  peer.to_json(writer);
  writer.Key("register_time");
  writer.Uint64(register_time);
  writer.Key("enable_monitor");
  writer.Bool(enable_monitor);
  writer.Key("packet_version");
  writer.Int((int)packet_version);
  writer.EndObject();
}

...
static void _list_oblogreader_processes(std::map<std::string, std::pair<int, uint32_t>>& process)
{
  ...
  if (ret != 0) {
    OMS_STREAM_ERROR << "Failed to list oblogreader process, ret " << ret
                     << ", error: " << (lines.empty() ? "" : lines[0]);
    return;
  }
...
static void _do_gc_child_path()
{
  ...
  for (auto& path_entry : paths) {
    ...
    if (now_tm - mod_time >= max_retain_seconds) {
      OMS_STREAM_WARN << "Died oblogreader path: " << path
                      << " exist time exceed max retain seconds: " << max_retain_seconds << ", about to delete";

...
#define OBCDC_CONFIG "ObcdcConfig"
#define BINLOG_CONFIG "BinlogConfig"
#define SLOT_CONFIG "SlotConfig"
#define CLIENT_META "ClientMeta"
#define INSTANCE_META "InstanceMeta"
...
class ConfigBase {
public:
  explicit ConfigBase(std::string name) : _name(std::move(name))
  {}
...
public:
  ...
  virtual void to_json(rapidjson::PrettyWriter<rapidjson::StringBuffer>& writer) const;

  virtual int from_json(const rapidjson::Value& value);
...
public:
  ...
  EncryptedConfigItem(ConfigBase* config, const std::string& key, const std::string& dft, bool optional = false,
      ...
      : ConfigItem<std::string>(config, key, dft, optional), _encrypt_key(std::move(encrypt_key))
  {
    _val = dft;
    config->add_item(key, this);
  }

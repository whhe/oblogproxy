...
struct ObcdcConfig : public ConfigBase {
public:
...
public:
  ObcdcConfig() : ConfigBase(OBCDC_CONFIG){};
  explicit ObcdcConfig(const std::string& str);

  void add(const std::string& key, const std::string& value);

  /**
   * add one or more items
   * @param str syntax: "k1=v1 k2=v2 k3=v3"
   */
  ...
  std::string to_string(bool formatted) const override;

  void to_json(rapidjson::PrettyWriter<rapidjson::StringBuffer>& writer) const override;

  int from_json(const rapidjson::Value& json) override;
...
void config_password(ObcdcConfig& config, bool is_child = true);
}  // namespace oceanbase::logproxy

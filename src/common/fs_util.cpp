...
bool FsUtil::remove(const std::string& dest, bool recursive)
{
  ...
  if (ret != 0) {
    OMS_ERROR("Failed to remove {}, ret: {}, errno: {}", dest, ret, system_err(errno));
    return false;
  }
  return true;
}
...
bool FsUtil::backup_dir(const std::string& src_dir, const std::string& dst_dir)
{
  std::error_code err_code;
  fs::copy(src_dir, dst_dir, fs::copy_options::recursive, err_code);
  if (err_code) {
    OMS_ERROR("Failed to copy directory from [{}] to [{}], error: {}", src_dir, dst_dir, err_code.message());
    return false;
  }

  OMS_INFO("Successfully copy directory from [{}] to [{}]", src_dir, dst_dir, err_code.message());
  return true;
}
...
int FsUtil::write_file(const std::string& filename, const std::string& content)
{
  ...
  defer(fclose_binary(fp));
  if (fp == nullptr) {
    OMS_ERROR("Failed to open {}, error: {}", filename, logproxy::system_err(errno));
    return OMS_FAILED;
  }
  if (!content.empty()) {
...
int64_t FsUtil::append_file(const std::string& file, std::string& content)
{
  FILE* fp = FsUtil::fopen_binary(file);
  defer(FsUtil::fclose_binary(fp));
  if (fp == nullptr) {
    OMS_ERROR("Failed to open file: {}", file);
    return OMS_FAILED;
  }

  int64_t ret = append_file(fp, content, content.size());
  return ret;
}
...
int64_t FsUtil::append_file(FILE* stream, std::string& content, size_t size)
{
  ...
  if (n != 1) {
    OMS_ERROR("Failed to append file, written: {}, expected: {}, error: {}", n, size, system_err(errno));
    return OMS_FAILED;
  }
  return n;
}
...
int FsUtil::rewrite(FILE* input, unsigned char* content, uint64_t start_pos, uint64_t len)
{
  ...
  OMS_DEBUG("current file offset: {}", ftell(input));
  if (ferror(input)) {
    OMS_ERROR("rewrite error: {}", logproxy::system_err(errno));
    return OMS_IO_ERROR;
  }
  fflush(input);
  return OMS_OK;
...
void FsUtil::seekg_last_line(FILE* fp, size_t& pos)
{
  if (fp != nullptr) {
    fseek(fp, 0, SEEK_END);
    char ch = ' ';
...
int64_t FsUtil::append_file(const std::string& file, MsgBuf& content)
{
  ...
  int ret = fclose(fp);
  OMS_DEBUG("Success to write file: {}, size: {} ret:{}", file, content.byte_size(), ret);
  return ret;
}
...
void FsUtil::clear_file(const std::string& filename)
{
  FILE* fp = fopen(filename.c_str(), "w");
  fclose(fp);
}

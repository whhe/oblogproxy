...
#define BINLOG_START_FIXED_POS 151
...
public:
  ...
  FormatDescriptionEvent(uint64_t timestamp);
...
private:
  ...
  // for inner use
  // e.g., 89fbcea2-db65-11e7-a851-fa163e618bac
  std::string _gtid_uuid_str_with_delimiter;

public:
  static int uuid_str_to_hex_bin(const std::string& uuid_str, unsigned char*& uuid_hex_bin);

  static std::string uuid_hex_bin_to_str(unsigned char*& uuid_hex_bin);

  void uuid_hex_bin_to_str_with_delim(std::string& uuid_str_with_delim);
...
public:
  ...
  std::string get_gtid_uuid_str_wtih_delimiter();
  ...
  uint64_t get_gtid_txn_id_intervals() const;

  void set_gtid_txn_id_intervals(uint64_t gtid_txn_id_intervals);

  std::string format_string();

  uint64_t get_gtid_length();

  void copy_from(const GtidMessage& gtid_message)
  {
    if (this != &gtid_message) {
      ...
      if (nullptr == _gtid_uuid) {
        OMS_ERROR("Failed to allocate memory for gtid_uuid");
      } else {
        memcpy(_gtid_uuid, gtid_message._gtid_uuid, SERVER_UUID_LEN);
      }

      _gtid_txn_id_intervals = gtid_message.get_gtid_txn_id_intervals();
      _txn_range = gtid_message._txn_range;
    }
  }
...
public:
  ...
  ~PreviousGtidsLogEvent() override;
  ...
  void get_gtid_messages(std::vector<GtidMessage*>& gtid_messages);
...
public:
  ...
  void get_next_previous_gtids(std::vector<GtidMessage*>& previous_gtids);

  void set_next_previous_gtids(std::vector<GtidMessage*>& previous_gtids);
...
private:
  ...
  uint64_t _index;
  // free when ~PreviousGtidsLogEvent
  std::vector<GtidMessage*> _next_previous_gtids;
  OP _op;
  bool _existed = false;
};
...
public:
  ...
  QueryEvent(const std::string& dbname, std::string sql_statment);
...
public:
  ...
  void set_partial_columns_bitmaps(unsigned char* partial_columns_bitmaps, size_t partial_cols_bytes);

  const unsigned char* get_partial_columns_bitmaps() const;
...
private:
  ...
  unsigned char* _partial_columns_bitmaps = nullptr;
  size_t _partial_cols_bytes = 0;
  MsgBuf _before_row;
  MsgBuf _after_row;
...
struct BinlogTrxOverview {
  bool has_previous_gtids_event = false;
...
int verify_event_checksum(uint64_t start_pos, FILE* fp, bool require_event_existed = true);

...
inline uint8_t json_diff_op(std::string& op_str)
{
  if (strcmp(op_str.c_str(), "replace") == 0) {
    return 0;
  } else if (strcmp(op_str.c_str(), "insert") == 0) {
    return 1;
  } else {
    return 2;
  }
...
public:
      ...
      char* header, const std::string& parent = "")
  {
    ...
    value.Accept(writer);
    OMS_DEBUG("Json type code: {}, value: {}", value.GetType(), sb.GetString());
    sb.Clear();
    int64_t offset = 0;
  ...
  static void parse_json_diff(const char* data, logproxy::MsgBuf& data_decode)
  {
    ...
    if (document.HasParseError()) {
      OMS_ERROR("Parse diff json error: {}, json: {}", document.GetParseError(), data);
      return;
    }
    ...
    OMS_DEBUG("Json diff value: {}", sb.GetString());

    if (!document.HasMember("diffs")) {
      OMS_ERROR("The format of json diff is illegal, json: {}", sb.GetString());
      return;
    }
    rapidjson::Value& diffs_json_arr = document["diffs"];
    if (!diffs_json_arr.IsArray()) {
      OMS_ERROR("The diff field of json diff is defined as an array illegal, but actually it is not, json: {}",
          sb.GetString());
      return;
    }

    for (auto& diff : diffs_json_arr.GetArray()) {
      // operation
      std::string op_str = diff["op"].GetString();
      uint8_t op_code = json_diff_op(op_str);
      auto* op_byte = static_cast<unsigned char*>(malloc(1));
      int1store(op_byte, op_code);
      data_decode.push_back(reinterpret_cast<char*>(op_byte), 1);

      // path length
      size_t path_size = diff["path"].GetStringLength();
      int path_bytes_len = logproxy::get_packed_integer(path_size);
      auto* path_len_bytes = static_cast<char*>(malloc(path_bytes_len));
      logproxy::write_lenenc_uint(path_len_bytes, MAX_PACKET_INTEGER_LEN, path_size);
      data_decode.push_back(path_len_bytes, path_bytes_len);

      // path value
      std::string path = diff["path"].GetString();
      char* path_value = static_cast<char*>(malloc(path_size));
      memcpy(path_value, path.c_str(), path_size);
      data_decode.push_back(path_value, path_size);
      OMS_DEBUG("operation: {}, path: {}", op_str, path);

      if (op_code == 2) {
        continue;
      }

      MsgBuf json_value_decode;
      parse_nested_object(diff["value"], json_value_decode, 1, nullptr);
      auto value_byte_size = json_value_decode.byte_size();

      int value_len = logproxy::get_packed_integer(value_byte_size);
      auto* value_len_bytes = static_cast<char*>(malloc(value_len));
      logproxy::write_lenenc_uint(value_len_bytes, MAX_PACKET_INTEGER_LEN, value_byte_size);
      data_decode.push_back(value_len_bytes, value_len);

      char* value_bytes = static_cast<char*>(malloc(value_byte_size));
      json_value_decode.bytes(value_bytes);
      data_decode.push_back(value_bytes, value_byte_size);
    }
  }
};

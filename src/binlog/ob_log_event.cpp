    ...
    : _flags(1)
{
  ...
  this->_server_id = s_meta.binlog_config()->server_id();
  OMS_DEBUG("[OblogEventHeader] type:{}, timestamp:{}, event_length: {}, next_position: {}, server_id: {}",
      type_code,
      timestamp,
      event_length,
      next_position,
...
OblogEventHeader::OblogEventHeader(EventType type_code)
    : _type_code(type_code), _timestamp(0), _event_length(0), _next_position(0)
{
  this->_server_id = s_meta.binlog_config()->server_id();
}
...
FormatDescriptionEvent::FormatDescriptionEvent(uint64_t timestamp)
...
size_t PreviousGtidsLogEvent::flush_to_buff(unsigned char* buff)
{
  ...
  for (auto const& gtid_it : gtid_msg) {
    ...
    // stored in left-closed-right-open format
    for (auto const& txn_range : msg->get_txn_range()) {
      int8store(buff + pos, txn_range.first);
      pos += 8;

      int8store(buff + pos, txn_range.second + 1);
      pos += 8;
    }
  }
...
void PreviousGtidsLogEvent::get_gtid_messages(std::vector<GtidMessage*>& gtid_messages)
{
  for (const auto& gtid_msg_pair : _gtid_messages) {
    auto* gtid_msg = new GtidMessage();
    gtid_msg->copy_from(*(gtid_msg_pair.second));
    gtid_messages.emplace_back(gtid_msg);
  }
}
...
void PreviousGtidsLogEvent::set_gtid_messages(const std::vector<GtidMessage*>& gtid_messages)
{
  for (GtidMessage* gtid_message : gtid_messages) {
    ...
    OMS_INFO("Assemble previous gtids UUID information: {}", uuid_str);
...
void PreviousGtidsLogEvent::deserialize(unsigned char* buff)
{
  ...
  for (uint64_t uuid_seq = 0; uuid_seq < get_gtid_uuid_num(); uuid_seq++) {
    ...
    for (uint64_t i = 0; i < gtid_message->get_gtid_txn_id_intervals(); ++i) {
      ...
      range.second = (txn_start != txn_end) ? txn_end - 1 : txn_end;
      txn_vector.emplace_back(range);
    }

    gtid_message->set_txn_range(txn_vector);
    OMS_INFO("[PreviousGtidsLogEvent] gtid: {}", gtid_message->format_string());
...
PreviousGtidsLogEvent::~PreviousGtidsLogEvent()
{
  for (auto gtid_message : _gtid_messages) {
    if (nullptr != gtid_message.second) {
...
void RotateEvent::set_next_previous_gtids(std::vector<GtidMessage*>& previous_gtids)
{
  _next_previous_gtids = previous_gtids;
}

void RotateEvent::get_next_previous_gtids(std::vector<GtidMessage*>& previous_gtids)
{
  previous_gtids = _next_previous_gtids;
}

QueryEvent::QueryEvent(const std::string& dbname, std::string sql_statment)
...
{
  return uuid_hex_bin_to_str(_gtid_uuid);
}

std::string GtidMessage::uuid_hex_bin_to_str(unsigned char*& uuid_hex_bin)
{
  std::string uuid_str;
  logproxy::dumphex(reinterpret_cast<const char*>(uuid_hex_bin), SERVER_UUID_LEN, uuid_str);
  return uuid_str;
}

std::string GtidMessage::get_gtid_uuid_str_wtih_delimiter()
{
  if (_gtid_uuid_str_with_delimiter.empty()) {
    uuid_hex_bin_to_str_with_delim(_gtid_uuid_str_with_delimiter);
  }
  return _gtid_uuid_str_with_delimiter;
}
...
uint64_t GtidMessage::get_gtid_txn_id_intervals() const
{
...
std::string GtidMessage::format_string()
{
  ...
  std::string uuid_str_with_delim;
  std::stringstream stream;
  // uuid
  //  stream << this->_gtid_uuid.substr(0,4)<<"-"<< this->_gtid_uuid.substr(4,2);

  uuid_hex_bin_to_str_with_delim(uuid_str_with_delim);
  stream << uuid_str_with_delim;

  for (auto& i : this->_txn_range) {
    if (i.second != i.first) {
...
void GtidMessage::uuid_hex_bin_to_str_with_delim(string& uuid_str_with_delim)
{
  std::string uuid_str;
  ...
  uuid_str_with_delim.append(item_first).append("-");
  pos += 8;

  std::string item_second = uuid_str.substr(pos, 4);
  transform(item_second.begin(), item_second.end(), item_second.begin(), ::tolower);
  uuid_str_with_delim.append(item_second).append("-");
  pos += 4;

  std::string item_third = uuid_str.substr(pos, 4);
  transform(item_third.begin(), item_third.end(), item_third.begin(), ::tolower);
  uuid_str_with_delim.append(item_third).append("-");
  pos += 4;

  std::string item_fourth = uuid_str.substr(pos, 4);
  transform(item_fourth.begin(), item_fourth.end(), item_fourth.begin(), ::tolower);
  uuid_str_with_delim.append(item_fourth).append("-");
  pos += 4;

  std::string item_fifth = uuid_str.substr(pos, 12);
  transform(item_fifth.begin(), item_fifth.end(), item_fifth.begin(), ::tolower);
  uuid_str_with_delim.append(item_fifth);
}

int GtidMessage::set_gtid_uuid(const std::string& gtid_uuid)
{
  return uuid_str_to_hex_bin(gtid_uuid, this->_gtid_uuid);
}

int GtidMessage::uuid_str_to_hex_bin(const std::string& uuid_str, unsigned char*& uuid_hex_bin)
{
  std::vector<std::string> parts;
  logproxy::split(uuid_str, '-', parts);
  if (parts.size() == 5) {
    uuid_hex_bin = static_cast<unsigned char*>(malloc(SERVER_UUID_LEN));
    int pos = 0;
    binlog::CommonUtils::hex_to_bin(parts[0], uuid_hex_bin + pos);
    pos += 4;
    binlog::CommonUtils::hex_to_bin(parts[1], uuid_hex_bin + pos);
    pos += 2;
    binlog::CommonUtils::hex_to_bin(parts[2], uuid_hex_bin + pos);
    pos += 2;
    binlog::CommonUtils::hex_to_bin(parts[3], uuid_hex_bin + pos);
    pos += 2;
    binlog::CommonUtils::hex_to_bin(parts[4], uuid_hex_bin + pos);
  } else {
    OMS_STREAM_ERROR << "Invalid server uuid:" << uuid_str;
    return OMS_FAILED;
  }
  return OMS_OK;
}
...
uint64_t GtidMessage::get_gtid_length()
{
  uint64_t len = 0;
  len += SERVER_UUID_LEN;
  len += 8;
...
void RowsEvent::set_partial_columns_bitmaps(unsigned char* partial_columns_bitmaps, size_t partial_cols_bytes)
{
  _partial_columns_bitmaps = partial_columns_bitmaps;
  _partial_cols_bytes = partial_cols_bytes;
}

const unsigned char* RowsEvent::get_partial_columns_bitmaps() const
{
  return _partial_columns_bitmaps;
}
...
size_t RowsEvent::flush_to_buff(unsigned char* buff)
{
  ...
  // shared_images only for PARTIAL_UPDATE_ROWS_EVENT
  if (get_header()->get_type_code() == PARTIAL_UPDATE_ROWS_EVENT) {
    assert(_partial_columns_bitmaps != nullptr);

    // options
    int1store(buff + pos, 1);
    pos += 1;
    // partial_columns
    memcpy(buff + pos, _partial_columns_bitmaps, _partial_cols_bytes);
    pos += _partial_cols_bytes;
  }
...
RowsEvent::~RowsEvent()
{
  ...
  if (nullptr != _partial_columns_bitmaps) {
    free(_partial_columns_bitmaps);
    _partial_columns_bitmaps = nullptr;
  }
...
int64_t seek_gtid_event(const std::string& binlog_file, std::vector<GtidLogEvent*>& gtid_log_events,
    bool& rotate_existed, uint8_t& checksum_flag, uint64_t& last_xid, std::vector<GtidMessage*>& previous_gtid_messages,
    int64_t& pos)
{
  unsigned char magic[BINLOG_MAGIC_SIZE];
  unsigned char header_buff[COMMON_HEADER_LENGTH];

  FILE* fp = logproxy::FsUtil::fopen_binary(binlog_file);
  defer(FsUtil::fclose_binary(fp));
  if (fp == nullptr) {
    OMS_ERROR("Failed to open file: {}, error: {}", binlog_file, logproxy::system_err(errno));
    return OMS_OK;
  }
  ...
  if (memcmp(magic, binlog_magic, BINLOG_MAGIC_SIZE) != 0) {
    OMS_ERROR("Invalid binlog file format: {}.", binlog_file);
    return OMS_FAILED;
  }
  pos += BINLOG_MAGIC_SIZE;

  int64_t file_size = FsUtil::file_size(binlog_file);
  OblogEventHeader header = OblogEventHeader();
  while (pos < file_size) {
    // read common header
    size_t ret = FsUtil::read_file(fp, header_buff, pos, COMMON_HEADER_LENGTH);
    if (ret != OMS_OK) {
      OMS_ERROR("Failed to read common header from pos: {}", pos);
      break;
    }
    header.deserialize(header_buff);
    switch (header.get_type_code()) {
      case XID_EVENT: {
        auto* buffer = static_cast<unsigned char*>(malloc(header.get_event_length()));
        FreeGuard<unsigned char*> free_guard(buffer);
        ret = FsUtil::read_file(fp, buffer, pos, header.get_event_length());
        if (ret != OMS_OK) {
          OMS_ERROR("Failed to read XID_EVENT from pos: {}", pos);
          break;
        }
        XidEvent xid_event;
        xid_event.deserialize(buffer);
        last_xid = xid_event.get_xid();
        break;
      }
      case GTID_LOG_EVENT: {
        auto* buffer = static_cast<unsigned char*>(malloc(header.get_event_length()));
        ...
        ret = FsUtil::read_file(fp, buffer, pos, header.get_event_length());
        if (ret != OMS_OK) {
          OMS_ERROR("Failed to read GTID_LOG_EVENT from pos: {}", pos);
          break;
        }
        auto* gtid_log_event = new GtidLogEvent();
        gtid_log_event->deserialize(buffer);
      ...
      case FORMAT_DESCRIPTION_EVENT: {
        ...
        ret = FsUtil::read_file(fp, buffer, pos, header.get_event_length());
        if (ret != OMS_OK) {
          OMS_ERROR("Failed to read FORMAT_DESCRIPTION_EVENT from pos: {}", pos);
          break;
        }
        auto format_description_event = FormatDescriptionEvent();
        format_description_event.deserialize(buffer);
        checksum_flag = format_description_event.get_checksum_flag();
        OMS_INFO("The checksum configuration of the current binlog file: {}", checksum_flag);
        break;
      }
      case PREVIOUS_GTIDS_LOG_EVENT: {
        ...
        ret = FsUtil::read_file(fp, buffer, pos, header.get_event_length());
        if (ret != OMS_OK) {
          OMS_ERROR("Failed to read PREVIOUS_GTIDS_LOG_EVENT from pos: {}", pos);
          break;
        }
        PreviousGtidsLogEvent previous_gtids_log_event;
        previous_gtids_log_event.deserialize(buffer);
        previous_gtids_log_event.get_gtid_messages(previous_gtid_messages);
        break;
      }
      default:
        break;
...
int seek_events(const std::string& binlog, std::vector<ObLogEvent*>& log_events, EventType event_type, bool single)
{
  ...
  if (memcmp(magic, binlog_magic, sizeof(magic)) != 0) {
    OMS_ERROR("The file format is invalid.");
    ifstream.close();
    return OMS_FAILED;
  }
...
{
  this->set_header(new OblogEventHeader(HEARTBEAT_LOG_EVENT, 0, 0, 0));
}
...
int verify_event_checksum(uint64_t start_pos, FILE* fp, bool require_event_existed)
{
  unsigned char header_buff[COMMON_HEADER_LENGTH];
  OblogEventHeader header = OblogEventHeader();
  size_t ret = FsUtil::read_file(fp, header_buff, start_pos, COMMON_HEADER_LENGTH);
  if (ret != OMS_OK) {
    OMS_ERROR("Failed to read common header from pos: {}", start_pos);
    return require_event_existed ? OMS_FAILED : OMS_OK;
  }
  header.deserialize(header_buff);
  OMS_INFO("Verified event header: {}", header.str_format());

  auto* buffer = static_cast<unsigned char*>(malloc(header.get_event_length()));
  FreeGuard<unsigned char*> free_guard(buffer);
  ret = FsUtil::read_file(fp, buffer, start_pos, header.get_event_length());
  if (ret != OMS_OK) {
    OMS_ERROR("Failed to read the binlog event to verify checksum, error: {}", system_err(errno));
    return OMS_FAILED;
  }

  uint32_t check_sum_pos = header.get_event_length() - 4;
  uint32_t check_sum_read = int4load(buffer + check_sum_pos);
  uint64_t crc = crc32(0L, Z_NULL, 0);
  uint32_t check_sum_compute = crc32(crc, buffer, check_sum_pos);
  if (check_sum_read != check_sum_compute) {
    OMS_ERROR(
        "The checksum [{}] carried by the binlog event from pos [{}] is different from the calculated checksum: {}",
        check_sum_read,
        start_pos,
        check_sum_compute);
    return OMS_FAILED;
  }
  OMS_INFO("The checksum carried by the binlog event from pos [{}] is equal to the calculated checksum: {} = {}",
      start_pos,
      check_sum_read,
      check_sum_compute);
  return OMS_OK;
...
int seek_and_verify_complete_trx(const std::string& binlog_file, BinlogTrxOverview& binlog_trx)
{
  unsigned char magic[BINLOG_MAGIC_SIZE];
  unsigned char header_buff[COMMON_HEADER_LENGTH];
  ...
  // read magic number
  int64_t cur_pos = 0;
  FsUtil::read_file(fp, magic, cur_pos, BINLOG_MAGIC_SIZE);
  if (memcmp(magic, binlog_magic, BINLOG_MAGIC_SIZE) != 0) {
    OMS_ERROR("Invalid binlog file format: {}.", binlog_file);
    return OMS_OK;
  }
  cur_pos += BINLOG_MAGIC_SIZE;

  int64_t last_event_start_pos_in_complete_trx = 0;
  int64_t file_size = FsUtil::file_size(binlog_file);
  OblogEventHeader header = OblogEventHeader();
  while (cur_pos < file_size) {
    size_t ret = FsUtil::read_file(fp, header_buff, cur_pos, COMMON_HEADER_LENGTH);
    if (ret != OMS_OK) {
      OMS_ERROR("Failed to read common header from pos: {}", cur_pos);
      break;
    }
    header.deserialize(header_buff);

    switch (header.get_type_code()) {
      case EventType::PREVIOUS_GTIDS_LOG_EVENT: {
        binlog_trx.has_previous_gtids_event = true;
        binlog_trx.last_complete_trx_end_pos = header.get_next_position();
        last_event_start_pos_in_complete_trx = cur_pos;
        break;
      }
      case EventType::GTID_LOG_EVENT: {
        auto* buffer = static_cast<unsigned char*>(malloc(header.get_event_length()));
        FreeGuard<unsigned char*> free_guard(buffer);
        ret = FsUtil::read_file(fp, buffer, cur_pos, header.get_event_length());
        if (ret != OMS_OK) {
          OMS_ERROR("Failed to read GTID_LOG_EVENT from pos: {}", cur_pos);
          break;
        }
        GtidLogEvent gtid_log_event;
        gtid_log_event.deserialize(buffer);

        binlog_trx.trx_need_truncated_gtid = gtid_log_event.get_gtid_txn_id();
        break;
      }
      case EventType::XID_EVENT:
      case EventType::QUERY_EVENT: {
        bool is_trx_end = true;
        if (EventType::QUERY_EVENT == header.get_type_code()) {
          auto* buffer = static_cast<unsigned char*>(malloc(header.get_event_length()));
          FreeGuard<unsigned char*> free_guard(buffer);
          ret = FsUtil::read_file(fp, buffer, cur_pos, header.get_event_length());
          if (ret != OMS_OK) {
            OMS_ERROR("Failed to read QUERY_EVENT from pos: {}", cur_pos);
            break;
          }
          QueryEvent query_event;
          query_event.deserialize(buffer);
          is_trx_end = strcmp(query_event.get_sql_statment().c_str(), BEGIN_VAR) == 0 ? false : true;
        }
    ...
    cur_pos = header.get_next_position();
  }

  if (binlog_trx.has_previous_gtids_event) {
    return verify_event_checksum(last_event_start_pos_in_complete_trx, fp);
  }
  return OMS_OK;
}

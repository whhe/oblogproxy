...
int recv_mysql_packet(int fd, int timeout, uint32_t& packet_length, uint8_t& sequence, MsgBuf& msgbuf)
{
  ...
  if (0 == ret) {
    OMS_INFO("Timeout when receiving mysql packet. time(in millisecond)={}", timeout);
    return OMS_TIMEOUT;
  }
  if (ret < 0) {
  ...
  if (ret != OMS_OK) {
    OMS_STREAM_ERROR << "Failed to read mysql handshake packet. length: " << packet_length
...
int send_mysql_packet(int fd, MsgBuf& msgbuf, uint8_t sequence)
{
  ...
  if (ret != OMS_OK) {
    OMS_ERROR("Failed to send packet, error: {}", system_err(errno));
    return ret;
  }
...
int MySQLInitialHandShakePacket::decode(const MsgBuf& msgbuf)
{
  ...
  if (ret != OMS_OK) {
    OMS_STREAM_ERROR
        << "Failed to skip the 'character set' and 'status flags' while decoding mysql InitialHandShakePacket";
    return ret;
  }
...
int MySQLHandShakeResponsePacket::encode(MsgBuf& msgbuf)
{
  ...
  if (capabilities_flag & CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA) {
    ...
    if (ret < 0) {
      OMS_STREAM_ERROR << "Failed to encode auth response data length: " << _auth_response.size()
                       << ", capacity last %d" << capacity - offset;
      return OMS_FAILED;
    }
    offset += ret;
  }
  ...
  //   and auth-response
    ...
    if (ret < 0) {
      OMS_STREAM_ERROR << "Failed to encode auth response data length: " << _auth_response.size() + 1
                       << ", capacity last " << capacity - offset;
      return OMS_FAILED;
    }
    offset += ret;
  }
...
bool MySQLResultSet::to_model(Model* model)
{
  if (col_count == 0) {
    return true;
  }
  ...
  if (model->model_type() == ModelTypeEnum::e().OBJECT) {
    ...
    ((Model*)model)->set_parse(rows[index].fields()[0]);
  }

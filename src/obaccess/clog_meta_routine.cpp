...
int ClogMetaRoutine::init(const ObcdcConfig& config)
{
  ...
  if (ret != OMS_OK) {
    OMS_STREAM_ERROR
        << "Failed to check the existence of svr_min_log_timestamp column in __all_virtual_server_clog_stat, "
           "disable clog check";
    _available = false;
  }
  return OMS_OK;
...
bool ClogMetaRoutine::check(const ObcdcConfig& config, std::string& errmsg)
{

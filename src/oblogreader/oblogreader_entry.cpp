...
int init_configs(char** argv, ObcdcConfig& obLogConfig, ClientMeta& client);
...
int init_configs(char** argv, ObcdcConfig& obLogConfig, ClientMeta& client)
{
  ...
  if ((!doc.HasMember(CONFIG) || !doc[CONFIG].IsObject() || doc[CONFIG].IsNull()) ||
      (!doc.HasMember(OBCDC_CONFIG) || !doc[OBCDC_CONFIG].IsObject() || doc[OBCDC_CONFIG].IsNull()) ||
  ...
  OMS_INFO("Parsed {}: {}", CONFIG, Config::instance().to_string(true));

  // deserialize OblogConfig
  if (OMS_OK != obLogConfig.from_json(doc[OBCDC_CONFIG])) {
    return OMS_FAILED;
  }
  OMS_INFO("Parsed {}: {}", OBCDC_CONFIG, obLogConfig.to_string(true));

...
TEST(COMMON, to_json)
{
  oceanbase::logproxy::Memory* memory_status = new Memory();
  memory_status->set_mem_used_ratio(2);
  OMS_STREAM_INFO << memory_status->serialize_to_json();

  oceanbase::logproxy::ProcessMetric process_metric;

  process_metric.set_memory_status(memory_status);
  //  memory.append(to_json(process_metric.memory_status).asString());
  OMS_STREAM_INFO << process_metric.serialize_to_json();
}

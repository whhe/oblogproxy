...
#include "binlog/instance_tasks.h"
#include "binlog/binlog_index.h"
#include "binlog/ob_log_event.h"
#include "binlog/binlog_state_machine.h"
...
protected:
  ...
  void SetUp() override
  {
    ...
    oblog_config.add("worker_path", binlog_log_bin_basename + "/" + cluster + "/" + tenant);
    meta = InstanceMeta();
    int pid = 0;
    InstanceAsynTasks::invoke_fork(binlog_log_bin_basename, 0, pid);
...
TEST_F(BinlogTest, test_all_type)
{
  ...
  BinlogIndexManager index_manager(std::string(".") + BINLOG_DATA_DIR + BINLOG_INDEX_NAME);
  index_manager.get_latest_index(index_record);

  //  assert(index_record._current_mapping.second == 2);

  sleep(60);
  std::vector<ObLogEvent*> log_events;
  OMS_STREAM_INFO << "seek events:" << index_record.get_file_name();
  seek_events(index_record.get_file_name(), log_events);

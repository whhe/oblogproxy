...
int Arranger::run_foreground()
{
  ...
  std::set<std::string> prefixs{"logproxy.2", "liboblog.log.2", "trace_record_read.2", "logproxy_"};
...
EventResult Arranger::on_handshake(const Peer& peer, const Message& msg)
{
  ...
  std::string errmsg;
  ObcdcConfig oblog_config(client.configuration);
  if (resolve(oblog_config, errmsg) != OMS_OK) {
    response_error(peer, msg.version(), ErrorCode::NO_AUTH, errmsg);
    return EventResult::ER_CLOSE_CHANNEL;
  }

  OMS_STREAM_INFO << "ObConfig from peer: " << peer.to_string() << " after resolve: " << oblog_config.to_string(false);
...
int Arranger::resolve(ObcdcConfig& hs_config, std::string& errmsg)
{
...
int Arranger::auth(const ObcdcConfig& oblog_config, std::string& errmsg)
{
...
int Arranger::create(ClientMeta& client, ObcdcConfig& oblog_config)
{
...
int Arranger::check_clog(const ObcdcConfig& oblog_config, std::string& errmsg)
{

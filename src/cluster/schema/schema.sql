...
(
    id           varchar(100) NOT NULL COMMENT 'uuid,节点唯一标识',
...
(
    node_id             varchar(100) NOT NULL COMMENT 'OBM的节点ID',
    ...
    INDEX obi_list (cluster, tenant)
);

CREATE TABLE IF NOT EXISTS `config_template`
(
    `id`          bigint UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'PK',
    `version`     varchar(32)              DEFAULT '1' COMMENT '@desc 配置的版本',
    ...
    gmt_modified  datetime        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`),
    CONSTRAINT `unique_config` UNIQUE (`key_name`, `granularity`, `scope`)
...
(
    ...
    gmt_create             datetime         NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    gmt_modified           datetime         NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    INDEX (`instance`, `commit_version_start`),
    PRIMARY KEY (`id`)
);
...
REPLACE INTO `user` (`username`, `tag`)
VALUES ('sys', '0');
REPLACE INTO `user` (`username`, `tag`)
VALUES ('admin', '1');
